#!/usr/bin/env python3
"""
PRD Generator - Comprehensive Product Requirements Document Creator
Generates professional PRDs with all required sections for feature development.
"""
...
class Colors:
    """Terminal color codes for enhanced UX"""
    HEADER = '\033[95m'
    BLUE = '\033[94m'
    CYAN = '\033[96m'
    GREEN = '\033[92m'
...
def load_context_from_file(file_path):
    ...
    Returns:
        dict: Parsed JSON context or empty dict on error
    """
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        print_colored(f"❌ Context file not found: {file_path}", Colors.RED)
        return {}
    except json.JSONDecodeError as e:
        print_colored(f"❌ Invalid JSON in context file: {e}", Colors.RED)
        return {}
    except Exception as e:
        print_colored(f"❌ Error loading context file: {e}", Colors.RED)
        return {}

def load_idea_from_file(file_path):
    """
    Load feature idea from text file
    ...
    try:
        ...
        print_colored(f"❌ Idea file not found: {file_path}", Colors.RED)
        return ""
    except Exception as e:
        print_colored(f"❌ Error loading idea file: {e}", Colors.RED)
        return ""

def interactive_mode():
    """
    Interactive mode for gathering feature idea and context
    
    Returns:
        tuple: (feature_idea, context_dict)
    """
    print_section("Interactive PRD Generator")
    print_colored("Please provide details for the PRD generation:\n", Colors.CYAN)
    
    # Get feature idea
    print_colored("Feature Idea (press Enter twice when done):", Colors.YELLOW)
    lines = []
    while True:
        try:
            line = input()
    ...
    if not feature_idea:
        print_colored("❌ No feature idea provided. Exiting.", Colors.RED)
        sys.exit(1)
    
    # Get optional context
    print_colored("\n\nAdditional Context (optional, JSON format, press Enter to skip):", Colors.YELLOW)
    print_colored("Example: {\"priority\": \"P0\", \"target_quarter\": \"Q2 2025\"}", Colors.CYAN)
    
    try:
        context_input = input().strip()
        context = json.loads(context_input) if context_input else {}
    except json.JSONDecodeError:
        print_colored("⚠️  Invalid JSON, skipping context", Colors.YELLOW)
        context = {}
    
    return feature_idea, context

def generate_prd(feature_idea, context=None):
    """
    Generate comprehensive PRD document
    
    Args:
        feature_idea (str): The feature idea/description
        context (dict): Additional context for the PRD
        
    Returns:
        str: Complete PRD document in markdown format
    """
    context = context or {}
    ...
    target_quarter = context.get('target_quarter', 'TBD')
    product_owner = context.get('product_owner', 'TBD')
    engineering_lead = context.get('engineering_lead', 'TBD')
...
- **End Users:** {context.get('target_users', 'All platform users')}
...
- Strategic alignment with Q{timestamp.year} roadmap
...
|---------|------|--------|---------|
| 1.0 | {timestamp.strftime('%Y-%m-%d')} | PRD Generator | Initial document creation |
...
*For questions or clarifications, contact: {product_owner}*
"""
    
    return prd_content

def main():
    """Main entry point for PRD generator"""
    parser = argparse.ArgumentParser(
        ...
        Examples:
          ...
          %(prog)s --idea "Add dark mode support" --output DARK_MODE_PRD.md
          
          # Generate PRD from file
          %(prog)s --file feature_idea.txt --output FEATURE_PRD.md
          
          # Interactive mode
          %(prog)s --interactive
          
          # With additional context
          %(prog)s --idea "User dashboard" --context '{"priority": "P0", "target_quarter": "Q2 2025"}'
          
          # Load context from file
          %(prog)s --idea "Analytics feature" --context context.json
        """)
    )
    ...
    input_group.add_argument(
        '-i', '--idea',
        type=str,
        help='Feature idea as a string'
    )
    input_group.add_argument(
        '-f', '--file',
        type=str,
        help='Path to file containing feature idea'
    )
    input_group.add_argument(
        '--interactive',
        action='store_true',
        help='Interactive mode for entering feature idea and context'
    )
    ...
    parser.add_argument(
        '-o', '--output',
        type=str,
        default='PRODUCT_REQUIREMENTS_DOCUMENT.md',
        help='Output file path (default: PRODUCT_REQUIREMENTS_DOCUMENT.md)'
    )
    ...
    parser.add_argument(
        '-c', '--context',
        type=str,
        help='Additional context as JSON string or path to JSON file'
    )
    
    args = parser.parse_args()
    
    # Print header
    print_section("PRD Generator - Interact Platform")
    print_colored("Generating comprehensive Product Requirements Document...\n", Colors.CYAN)
    ...
    try:
        if args.interactive:
            feature_idea, context = interactive_mode()
        elif args.idea:
            feature_idea = args.idea
        elif args.file:
            feature_idea = load_idea_from_file(args.file)
            if not feature_idea:
                print_colored("❌ Failed to load feature idea from file", Colors.RED)
                sys.exit(1)
        
        # Get context
        if args.context and not args.interactive:
            ...
            if os.path.isfile(args.context):
                ...
                try:
                    ...
                    print_colored(f"⚠️  Invalid JSON context: {e}", Colors.YELLOW)
                    print_colored("Proceeding without additional context...", Colors.YELLOW)
        
        # Validate feature idea
        if not feature_idea or len(feature_idea.strip()) < 10:
            print_colored("❌ Feature idea must be at least 10 characters", Colors.RED)
            sys.exit(1)
        
        # Generate PRD
        print_colored(f"\n📝 Generating PRD for: {feature_idea[:50]}...", Colors.CYAN)
        print_colored(f"📄 Output file: {args.output}", Colors.CYAN)
        
        prd_content = generate_prd(feature_idea, context)
        ...
        # Success message
        print_colored("\n" + "="*60, Colors.GREEN)
        print_colored("✅ PRD Generated Successfully!", Colors.BOLD + Colors.GREEN)
        print_colored("="*60, Colors.GREEN)
        print_colored(f"\n📁 File: {output_path.absolute()}", Colors.GREEN)
        print_colored(f"📊 Size: {file_size:,} bytes ({file_size/1024:.1f} KB)", Colors.GREEN)
        print_colored(f"📝 Lines: {prd_content.count(chr(10)) + 1}", Colors.GREEN)
        
        print_colored("\n📋 Document Sections:", Colors.CYAN)
        print_colored("  ✓ Executive Summary", Colors.GREEN)
        print_colored("  ✓ Problem Statement", Colors.GREEN)
        print_colored("  ✓ Target Audience / User Personas (3 personas)", Colors.GREEN)
        print_colored("  ✓ Functional Requirements", Colors.GREEN)
        print_colored("  ✓ Non-Functional Requirements", Colors.GREEN)
        print_colored("  ✓ User Stories & Acceptance Criteria (Gherkin)", Colors.GREEN)
        print_colored("  ✓ Technical Architecture Overview", Colors.GREEN)
        print_colored("  ✓ API Design (5 endpoints)", Colors.GREEN)
        print_colored("  ✓ UI/UX Considerations", Colors.GREEN)
        print_colored("  ✓ Security & Compliance (GDPR, SOC 2, HIPAA)", Colors.GREEN)
        print_colored("  ✓ Testing Strategy (Vitest, Playwright)", Colors.GREEN)
        print_colored("  ✓ Deployment & DevOps Plan (3 environments)", Colors.GREEN)
        print_colored("  ✓ Assumptions, Risks & Open Questions", Colors.GREEN)
        
        print_colored("\n🎯 Next Steps:", Colors.YELLOW)
        print_colored("  1. Review and customize the generated PRD", Colors.YELLOW)
        print_colored("  2. Add specific technical details", Colors.YELLOW)
        print_colored("  3. Share with stakeholders for feedback", Colors.YELLOW)
        print_colored("  4. Update timeline and resource estimates", Colors.YELLOW)
        print_colored("  5. Get approval before development starts", Colors.YELLOW)
        ...
        print_colored(f"\n❌ Error generating PRD: {e}", Colors.RED)
        import traceback
        if os.getenv('DEBUG'):
            traceback.print_exc()
        return 1
